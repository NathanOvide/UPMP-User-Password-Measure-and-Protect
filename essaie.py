...
fenetre.iconbitmap('icon.ico')                                                                                          # Icon de la fenêtre
w = 620                                                                                                                 #taille souhaite de la fenetre
h = 210
ws = fenetre.winfo_screenwidth()                                                                                        #pour centrer la fenetre
hs = fenetre.winfo_screenheight()                                                                                       #taille de l'ecran
x = (ws/2) - (w/2)                                                                                                      #calcul la position de la fenetre
y = (hs/2) - (h/2)
fenetre.geometry('%dx%d+%d+%d' % (w, h, x, y))                                                                          # Taille de la fenêtre
fenetre.configure(bg="white")                                                                                           # Couleur Background fenêtre
fenetre.resizable(width=False, height=False)                                                                            # La fenetre n'est pas redimensionnable
...
def window_personal_info(window_personal_info):
    ...
    def prenom_individu():
        global LePrenomI                                                                                                # Modification apporté soit valable partout
        global MotDePasseI
        prenomi = entry_prenom.get()                                                                                    # Demande du prénom de l'individu qui s'enregistre directement enminuscule
        var = str.lower(prenomi)
        ...
        MotDePasseI.append(str.lower(prenomi))                                                                          # La liste des mot de passes intègre la variable en majuscules
        ...
        for a in range(len(prenomi)):                                                                                   # Création de Mot de passe avec 1 lettre majuscule constament
            ...
            if LePrenomI[a] >= 97 and LePrenomI[a] <= 122:                                                              # Si le nombre de la variable a est comprise entre 97 et 122 inclus alors
                ...
                MotDePasseI.append("".join(LePrenomI))                                                                  # Le mot de passe l'intègre ayant a en majuscule
                for z in range(len(prenomi)):
                    if z != a:                                                                                          # Lorsque z est différent de la valeur a
                        LePrenomI[z] = ord(LePrenomI[z])                                                                # on met au fur et à mesure les autres lettre en majuscule
                        if LePrenomI[z] >= 97 and LePrenomI[z] <= 122:                                                  # Tout en gardant la variable a en majuscule
                            ...
                            LePrenomI[z] = ord(LePrenomI[z])                                                            # a reste toujours en majusculke, en revancge toutes les autres redeviennent en minuscule
                            LePrenomI[z] = chr(LePrenomI[z] + 32)

        for a in range((len(prenomi))-1):                                                                               # Création de Mot de passe avec 2 lettres majuscule constament
            b = a + 1
            LePrenomI = [elt for elt in var]
            LePrenomI[a] = ord(LePrenomI[a])
            LePrenomI[b] = ord(LePrenomI[b])
            if LePrenomI[a] >= 97 and LePrenomI[a] <= 122 and LePrenomI[b] >= 97 and LePrenomI[b] <= 122:
                LePrenomI[a] = chr(LePrenomI[a] - 32)
                LePrenomI[b] = chr(LePrenomI[b] - 32)
                MotDePasseI.append("".join(LePrenomI))
                for p in range(len(prenomi)):
                    if z != a and z != b:
                        LePrenomI[z] = ord(LePrenomI[z])
                        if LePrenomI[z] >= 97 and LePrenomI[z] <= 122:
                            LePrenomI[z] = chr(LePrenomI[z] - 32)
                            MotDePasseI.append("".join(LePrenomI))
                            LePrenomI[z] = ord(LePrenomI[z])
                            LePrenomI[z] = chr(LePrenomI[z] + 32)

        for a in range((len(prenomi)) - 2):                                                                             # Création de Mot de passe avec une lettre majuscule constament
            b = a + 1
            c = a + 2
            LePrenomI = [elt for elt in var]
            LePrenomI[a] = ord(LePrenomI[a])
            ...
            if LePrenomI[a] >= 97 and LePrenomI[a] <= 122 and LePrenomI[b] >= 97 and LePrenomI[b] <= 122 and LePrenomI[c] >= 97 and LePrenomI[c] <=122:
                LePrenomI[a] = chr(LePrenomI[a] - 32)
                LePrenomI[b] = chr(LePrenomI[b] - 32)
        ...
        for a in range((len(prenomi)) - 3):                                                                             # Création de Mot de passe avec une lettre majuscule constament
            b = a + 1
            c = a + 2
            d = a + 3
            LePrenomI = [elt for elt in var]
            ...
            if LePrenomI[a] >= 97 and LePrenomI[a] <= 122 and LePrenomI[b] >= 97 and LePrenomI[b] <= 122 and LePrenomI[c] >= 97 and LePrenomI[c] <= 122 and LePrenomI[d] >= 97 and LePrenomI[d] <= 122:
                LePrenomI[a] = chr(LePrenomI[a] - 32)
                LePrenomI[b] = chr(LePrenomI[b] - 32)
                LePrenomI[c] = chr(LePrenomI[c] - 32)
    ...
    texte_nom = Label(window_personal_info, text="Nom :", font=Banhschrift)
    ...
    texte_datenaissance = Label(window_personal_info, text="Date de Naissance :", font=Banhschrift)
    texte_datenaissance.pack(anchor="w")
    texte_datenaissance.place(x=20, y=160)

    # ----------

    jourMenu = ttk.Combobox(window_personal_info)
    jourMenu.place(x=180, y=165, width=45)
    jourMenu['values'] = (
    ...
    def jour_individu():
        ...
        JourNaissance = jourMenu.get()
        MotDePasseDNI.append(JourNaissance)
    ...
    moisMenu = ttk.Combobox(window_personal_info)
    moisMenu.place(x=238, y=165, width=50)
    moisMenu["values"] = ("Mois", "01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12")
    moisMenu.current(0)

    def mois_individu():
        global MotDePasseDNI
        MoisNaissance = moisMenu.get()
        MotDePasseDNI.append(MoisNaissance)
    ...
    anneeMenu = ttk.Combobox(window_personal_info)
    anneeMenu.place(x=301, y=165, width=62)
    anneeMenu["values"] = (
    ...
    def annee_individu():
        global MotDePasseDNI
        AnneeNaissance = anneeMenu.get()
        MotDePasseDNI.append(AnneeNaissance)

    # ------------------------------

    texte_lieunaissance = Label(window_personal_info, text="Ville Natale :", font=Banhschrift)
    texte_lieunaissance.pack(anchor="w")
    texte_lieunaissance.place(x=20, y=210)

    lieunaissanceMenu = Entry(window_personal_info, width=30, bg='white', fg="black", justify="center")
    lieunaissanceMenu.place(x=180, y=215)

    def lieu_individu():
        global LeLieuDeNaissance
        LieuNaissance = lieunaissanceMenu.get()
        LeLieuDeNaissance.append(LieuNaissance)

    # ------------------------------

    texte_departementnaissance = Label(window_personal_info, text="Département de Naissance :", font=Banhschrift)
    texte_departementnaissance.pack(anchor="w")
    texte_departementnaissance.place(x=20, y=260)

    numdepartementMenu = ttk.Combobox(window_personal_info)
    numdepartementMenu.place(x=239, y=263, width=125)
    ...
    def dlieu_individu():
        ...
        DLieuNaissance = numdepartementMenu.get()
        LeDLieuDeNaissance.append(DLieuNaissance)

    # ------------------------------

    def creation_MDP_individu():
        global MotDePasseI
        global MotDePasseDNI
        global LeLieuDeNaissance
        global LeDLieuDeNaissance
        ...
        lieu_individu()
        dlieu_individu()
        for i in range(len(MotDePasseI)):
...
def window_parent1_info(window_parent1_info):
    ...
    window_parent1_info.title("Votre parent 1 : ")                                                                      # Titre de la nouvelle fenêtre1
...
def window_parent2_info(window_parent2_info):
    ...
    window_parent2_info.title("Votre parent 2 : ")                                                                      # Titre de la nouvelle fenêtre1
...
def window_conjoint_info(window_conjoint_info):
    ...
    window_conjoint_info.title("Votre conjoint : ")                                                                     # Titre de la nouvelle fenêtre1
...
def window_freresoeur_info(window_freresoeur_info):
    ...
    window_freresoeur_info.title("Vos frères et sœurs : ")                                                              # Titre de la nouvelle fenêtre1
...
def window_animaux_info(window_conjoint_info):
    ...
    window_animaux_info.title("Vos animaux : ")                                                                         # Titre de la nouvelle fenêtre1
    ...
    texte_chien = Label(window_animaux_info, text="Chien : ", font='Helvetica, 11 ')
    ...
    texte_chat = Label(window_animaux_info, text="Chat : ", font='Helvetica, 11 ')
    ...
    texte_cheval = Label(window_animaux_info, text="Cheval : ", font='Helvetica, 11 ')
    ...
    texte_lapin = Label(window_animaux_info, text="Lapin : ", font='Helvetica, 11 ')
    ...
    texte_oiseau = Label(window_animaux_info, text="Oiseau : ", font='Helvetica, 11 ')
    ...
    texte_poisson = Label(window_animaux_info, text="Poisson : ", font='Helvetica, 11 ')
    ...
    def race():
        global MotDePasseA
        Chat = entry_chat.get()
        MotDePasseA.append(Chat)
        MotDePasseA.append(str.upper(Chat))
        MotDePasseA.append(str.lower(Chat))
...
def window_mdp_fin():
    window_mdp_fin = Toplevel()                                                                                         # Nouvelle fenetre1 au dessus de la précédente
    ...
    # ------------------------------
    texte_info = Label(window_mdp_fin, text="Bravo ! Vous avez atteint la dernière étape.", font='Helvetica, 12')
    texte_info.pack(anchor="w")
    texte_info.place(x=250, y=8)

    texte_info1 = Label(window_mdp_fin,
                       text="Elle consite à la vérification de votre mot de passe. \nPour cela, il vous faut entrer votre mot de passe dans la zone de texte et nous vous dirons si celui-ci est fiable ou non. \nVous aurez par la suite la possibilité d'en proposer un autre.",
                       font='Helvetica, 11')
    texte_info1.pack(anchor="w")
    texte_info1.place(x=20, y=30)

    # ----------

    texte_mdp = Label(window_mdp_fin, text="Proposez un mot de passe : ", font='Bahnschrift, 13')
    texte_mdp.pack(anchor="w")
    texte_mdp.place(x=150, y=120)

    entry_mdp = Entry(window_mdp_fin, width=30, bg='white', fg="black",justify="center")                                # Zone entrée qui récupère le choix utlisateur de l'élément dans la liste + taille + couleur fond
    ...
    def check():
        ...
        if Resultat > 0:
            showinfo('Résultat : ', 'Votre mot de passe n\'est pas fiable ou pas assez. ')
        if Resultat == 0:
            showinfo('Résultat : ', 'Votre mot de passe est fiable ou assez fiable. ')
...
def web():
    ...
    url = 'www.google.com'                                                                                              #URL de requête
...
def quit():
    ...
    if askyesno('Quitter ?', 'Êtes-vous sür de vouloir quitter ?'):                                                     #Fenetre de dialogue (OUI-NON)
...
heure = StringVar()
fenetre_heure = Label(fenetre, textvariable=heure)
fenetre_heure.place(x=300, y=171)
fenetre_heure.configure(bg="white")
...
listbox.config(bg="gray90", relief="groove", bd=7)                                                                      # Couleur fond + relief éléments liste + marge intérieur + curseur souris
...
entrylist = Entry(fenetre, width=50, bg='blue', fg="white", justify="center")                                           # Zone entrée qui récupère le choix utlisateur de l'élément dans la liste + taille + couleur fond
entrylist.insert(0, 'Sélectionnez un élément de la liste')                                                              # Texte entré de base dans la zone
entrylist.pack(anchor="nw", pady=12)                                                                                    # Postionnement de la zone




def warningbox():                                                                                                       # Ouverture boîte de dialogue signalant erreur de sélection
    showwarning("Sélection Impossible", "Veuillez choisir une sélection valide \ndans l'encadré de gauche.")



def showinfobox_fraternite():                                                                                           # Ouverture boîte de dialogue signalant des informations sur la fraternité
    showinfo('Informations : ', 'Si vous avez plusieurs frères et soeurs, merci d\'appuyer sur le bouton \"Valider les informations\" entre chaque frère et soeur.')

def showinfobox_animaux():                                                                                              # Ouverture boîte de dialogue signalant des informations sur la fraternité
    showinfo('Informations : ', 'Si vous avez plusieurs animaux, merci d\'appuyer sur le bouton \"Valider les informations\" entre chaque animaux.')
...
bouton_valider = Button(fenetre, text= "      Entrer les      \n informations", command=ouverture_fenetre, relief="groove")   #Bouton avec texte + commande
bouton_valider.pack(anchor="sw")
bouton_valider.place(x=410, y=60)
# Postion bouton Gauche + remplissage hauteur + distance par rapport aux autres élements

bouton_terminer = Button(fenetre, text=" Terminer ", command=window_mdp_fin, relief="groove")
bouton_terminer.pack(anchor="sw")
bouton_terminer.place(x=560, y=170)
